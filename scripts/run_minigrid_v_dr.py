...
PARAMS = {
    ...
    # PLR — degraded to uniform random (no prioritization)
    'use_plr':                      True,
    'level_replay_strategy':        'random',   # uniform sampling, no scoring
    'level_replay_score_transform': 'rank',
    'level_replay_temperature':     0.1,
    'staleness_coef':               0.0,        # no staleness bonus
    'use_reset_random_dr':          True,       # always generate new levels, bypass replay buffer
...
if __name__ == '__main__':
    ...
    if args.method is None:
        args.method = f'MultiGrid-{variant}-DR'
    if args.exp_name is None:
        args.exp_name = f'minigrid_{variant.lower()}_dr'
