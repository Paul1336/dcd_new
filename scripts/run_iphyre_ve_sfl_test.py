"""
Quick smoke-test for the iphyre-ve-sfl pipeline (VLM Embedding + SFL).

Loads only 20 VLM tasks, uses 2 parallel envs, and stops after ~16 training
iterations so the full end-to-end flow (env creation → image render → CLIP
encode → rollout → PPO update → learnability → eval) can be verified in a
few minutes on any machine.

CLIP runs on CPU so no GPU is required for the smoke-test.

Usage:
    cd dcd_new
    python scripts/run_iphyre_ve_sfl_test.py [--device cuda:0] [--log_dir ~/logs/dcd/]
"""
...
PARAMS = {
    ...
    # VLM Embedding (CLIP)
    # clip_device=cpu: no GPU required for the smoke-test; CLIP on CPU is fast
    # enough for 2 envs and a handful of iterations.
    'obs_type':                             'embedding',
    'clip_model':                           'ViT-B/32',
    'clip_device':                          'cpu',
}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method',   type=str, default='Iphyre-VE-SFL-test')
    parser.add_argument('--exp_name', type=str, default='iphyre_ve_sfl_test')
    args = parser.parse_args()

    cmd = build_cmd(args.device, args.log_dir, args.method, args.exp_name)

    print('=== smoke-test (1 trial, seed=88, 20 tasks, ~16 iters, CLIP on CPU) ===\n')
    print(cmd)
    print()
