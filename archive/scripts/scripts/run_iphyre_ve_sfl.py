...
# VLM-Embedding SFL: identical to V-SFL except obs_type="embedding" causes
# the env to render images (state_type="image") and a frozen CLIP encoder
# (VecCLIPEmbeddingWrapper) to convert them to 512-dim float vectors before
# the agent.  clip_device should match --device so the GPU budget is shared
# between PPO and CLIP; CLIP encode is lightweight relative to PPO.
PARAMS = {
    # Env
    ...
    'obs_type':                             'embedding',
    'clip_model':                           'ViT-B/32',
    # clip_device is set dynamically to match --device (see build_cmd)
}
...
def build_cmd(seed, device, log_dir, method, exp_name, extra_params=None):
    params = dict(PARAMS)
    # Default clip_device to match the training device so both share the same GPU.
    params['clip_device'] = device
    if extra_params:
