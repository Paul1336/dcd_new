...
class AdversarialRunner(object):
    ...
    def __init__(
        ...
        self.is_sfl = args.ued_algo == 'sfl'
        self.is_old_sfl = args.ued_algo == 'old_sfl'
        ...
        self.learnability_sampler = LearnabilitySampler(
            ...
            ued_algo=args.ued_algo
        )

        self.env_sampling_total_count = {}
        self.env_sampling_current_count = {}
    ...
    def _get_env_stats(self, agent_info, adversary_agent_info, log_replay_complexity=False):
        ...
        elif env_name.startswith('Iphyre'):
            stats = {}
        else:
    ...
    def agent_rollout(self, 
                      ...
                      num_steps, 
                      update=False, 
                      is_env=False,  # is true for the adversary rollout
                      level_replay=False, 
                      level_sampler=None, 
        ...
        # AdversaryEnv rollout -- if it is a method that doesn't require to train a adversary agent, it returns early.
        if is_env:
            if edit_level: # Get mutated levels
                ...
                return
            elif self.is_sfl or self.is_old_sfl:
                sampled_levels = [self.learnability_sampler.sample() for _ in range(args.num_processes)]
                self.ued_venv.reset_to_level_batch(sampled_levels)

                print('sampled_levels: ', sampled_levels)
                print('len(sampled_levels): ', len(sampled_levels))
                return
            ...
            # If we don't have use_reset_random_dr, then it will continue to run adversary step_adversary(). 
        ...
        for step in range(num_steps):
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    ...
                    if env_name not in self.env_sampling_total_count:
                        self.env_sampling_total_count[env_name] = 0
                    if env_name not in self.env_sampling_current_count:
                        self.env_sampling_current_count[env_name] = 0
                    ...
                    if not is_env:
                        ...
                        # Note that I think this implementation is different from the paper.
                        # In the paper, it always sample the next level after episode ends.
                        # But here, it only sample a new level if level_replay is True.
                        if level_sampler and level_replay:
                            if self.use_accel_paired:
                                ...
                                level = self.level_store.get_level(level_seed)
                                print('After episode ended, we reset to level: ', level_seed, level)
                                obs_i = self.venv.reset_to_level(level, i)
                                set_obs_at_index(obs, obs_i, i)
                                next_level_seeds[i] = level_seed
                                rollout_info['solved_idx'][i] = True
                        
                        if self.is_sfl or self.is_old_sfl:
                            sampled_level = self.learnability_sampler.sample()
                            # print('After episode ended, we reset to level: ', sampled_level)
                            obs_i = self.venv.reset_to_level(sampled_level, i)
                            set_obs_at_index(obs, obs_i, i)
                            # rollout_info['solved_idx'][i] = True
    ...
    def _update_learnability_metrics_old_sfl(self, global_step):
        from envs.iphyre.adversarial import generate_task, rand_int_seed
        from eval import evaluate_parallel_envs

        print('Update Learnability for old SFL...')
        print('global_step: ', global_step)
        print('staleness: ', self.args.learnability_staleness)
        print('buffer_size: ', self.args.sfl_buffer_n_size)
        print('buffer_nl_size: ', self.args.sfl_buffer_nl_size)
        print('top_k_to_sample_uniformly: ', self.args.top_k_to_sample_uniformly)
        print('env_names: ', self.learnability_sampler.env_names)

        # Sample N random levels
        sampled_env_names = []
        print(f'Sampling {self.args.sfl_buffer_n_size} random levels...')
        for index in tqdm(range(self.args.sfl_buffer_n_size), desc='Sampling random levels'):
            task_config = generate_task()
            env_name = rand_int_seed()
            sampled_env_names.append(env_name)
            
            PARAS[env_name] = task_config

        # Rollout policy for all N levels
        chunk_size = 40
        env_names_chunks = [
            sampled_env_names[i : i + chunk_size]
            for i in range(0, len(sampled_env_names), chunk_size)
        ]

        print(f"Evaluating {self.args.sfl_buffer_n_size} levels...")
        all_learnability_scores = []
        for env_names_chunk in tqdm(env_names_chunks, desc='Evaluating levels'):
            env_eval_results = evaluate_parallel_envs(
                env_names=env_names_chunk,
                env_task_configs=[None for _ in env_names_chunk],
                agent=self.agents['agent'],
                num_episodes=10,
                device=self.device
            )

            for env_id in env_names_chunk:
                zero_shot_success_rate = env_eval_results[env_id]["success_rate"]
                learnability_score = zero_shot_success_rate * (1 - zero_shot_success_rate)
                all_learnability_scores.append(
                    {
                        "score": learnability_score,
                        "env_id": env_id,
                        "success_rate": zero_shot_success_rate,
                    }
                )

        sorted_learnability_scores = sorted(all_learnability_scores, key=lambda x: x['score'], reverse=True)

        if self.args.top_k_to_sample_uniformly is None:
            raise ValueError("top_k_to_sample_uniformly must be set for old SFL")
        top_k_env_scores = sorted_learnability_scores[:self.args.top_k_to_sample_uniformly]
        print(f'Top {self.args.top_k_to_sample_uniformly} levels by learnability score:')

        # for env_score in top_k_env_scores:
        #     print(f'topk: {env_score["env_id"]} with zero-shot success rate {env_score["success_rate"]}')

        sampled_env_scores_top_k = np.random.choice(top_k_env_scores, size=int(self.args.sfl_buffer_nl_size* self.args.learnability_staleness) , replace=False)
        print(f'Sampled {len(sampled_env_scores_top_k)} levels from top {self.args.top_k_to_sample_uniformly} levels by learnability score')

        # Additionally, sample N_L * (1 - rho) levels randomly.
        random_sampled_env_names = []
        for index in range(int(self.args.sfl_buffer_nl_size * (1 - self.args.learnability_staleness))):
            task_config = generate_task()
            env_name = rand_int_seed()
            PARAS[env_name] = task_config
            random_sampled_env_names.append(env_name)
        
        new_env_names = []
        for env_score in sampled_env_scores_top_k:
            new_env_names.append(env_score['env_id'])
            print(f'Sampled {env_score["env_id"]} with zero-shot success rate {env_score["success_rate"]}')
        for env_name in random_sampled_env_names:
            new_env_names.append(env_name)

        print(f'Sampled {len(random_sampled_env_names)} from random levels')
        print(f'Sampled {len(new_env_names)} levels in total')

        self.learnability_sampler.update_env_names(new_env_names)


        # Save those envs to a file
        os.makedirs(f"{self.args.log_dir}/learnability", exist_ok=True)
        with open(
            f"{self.args.log_dir}/learnability/sfl_buffer_{global_step}.json", "w"
        ) as f:
            task_config_dict = {}
            for env_name in new_env_names:
                task_config_dict[env_name] = PARAS[env_name]
            task_config_dict["top_k_levels"] = top_k_env_scores
            task_config_dict["random_levels"] = random_sampled_env_names
            json.dump(task_config_dict, f)



    def _update_learnability_metrics(self, global_step):
        env_names = self.learnability_sampler.env_names

        print('Update Learnability...', self.args.log_dir)

        os.makedirs(f"{self.args.log_dir}/learnability", exist_ok=True)

        task_info_dict = copy.deepcopy(self.learnability_sampler.task_info_dict)
        for env_id in env_names:
            if env_id in self.env_sampling_current_count:
                task_info_dict[env_id]["sampling_count"] = self.env_sampling_current_count[
                    env_id
                ]

                # Reset the sampling count for the next iteration
                self.env_sampling_current_count[env_id] = 0

        with open(
            f"{self.args.log_dir}/learnability/learnability_{global_step}.json", "w"
        ) as f:
            json.dump(task_info_dict, f)

        print(
            f"Saving learnability to {f'{self.args.log_dir}/learnability/learnability_{global_step}.json'}"
        )

        with open(
            f"{self.args.log_dir}/learnability/env_sampling_total_count.json", "w"
        ) as f:
            json.dump(self.env_sampling_total_count, f)
        print(
            f"Saving env sampling total count to {f'{self.args.log_dir}/learnability/env_sampling_total_count.json'}"
        )
    
        # subsample to learnability buffer size
        learnability_buffer_size = self.args.learnability_buffer_size
        sampled_env_ids = random.sample(env_names, learnability_buffer_size)

        from eval import evaluate_parallel_envs

        chunk_size = 40
        env_ids_chunks = [
            sampled_env_ids[i : i + chunk_size]
            for i in range(0, len(sampled_env_ids), chunk_size)
        ]

        from tqdm import tqdm

        for env_ids_chunk in tqdm(env_ids_chunks, desc="Updating learnability"):
            env_eval_results = evaluate_parallel_envs(
                env_names=env_ids_chunk,
                env_task_configs=[None for _ in env_ids_chunk],
                agent=self.agents['agent'],
                num_episodes=10,
                device=self.device
            )

            for env_id in env_ids_chunk:
                self.learnability_sampler.update_learnability(
                    env_id,
                    global_step,
                    env_eval_results[env_id]["success_rate"],
                )


    def run(self, global_step, iteration, total_iterations):
        args = self.args
        ...
        # See paper: Robust PLR. If we do gradient update on new random levels, it could make the policy less robust in minimax terms.
        ...
        if self.is_old_sfl and self.is_training and (iteration % args.update_learnability_every_iterations == 0 or iteration == 1):
            self._update_learnability_metrics_old_sfl(global_step)

        # SFL: Calculate the learnability metrics for the training levels of VLM generated tasks.
        if self.is_sfl and self.is_training and (iteration % args.update_learnability_every_iterations == 0 or iteration == 1):
        ...
        if self.is_training:
            ...
            lrnow = frac * args.lr
            agent.update_lr(lrnow)
        
        # Generate a batch of adversarial environments
        # PAIRED: Use adversary to generate environment parameters: ~θ ∼  ̃Λ. Use to create POMDP M~θ .
        ...
        # PAIRED: Collect Protagonist trajectory τ P in POMDP M~θ .
        agent_info = self.agent_rollout(
            ...
            update_agent_separately=self.use_accel_paired) # Most likely false because self.use_accel_paired is false except one config.
        ...
        if self.is_paired:
            ...
            # PAIRED: Collect Antagonist trajectory τ A in POMDP M~θ .
            # Basically the same as protagonist rollout, but using adversary_agent instead of agent.
        ...
        print('level_replay: ', level_replay, 'edit_level: ', edit_level)
        ...
        if level_replay and edit_level:
            ...
            # TODO: what is fixed_seets?
            # Why env_info looks like a seed?
            print('ACCEL editing..., env_info: ', env_info)
            print('fixed_seeds: ', fixed_seeds)


            # TODO: what is is_updateable? 
            level_sampler, is_updateable = self._get_level_sampler('agent')
        ...
        stats.update({
            ...
            'agent_approx_kl': agent_info['update_info'].get('approx_kl', None),
            'agent_used_epoch': agent_info['update_info'].get('used_epoch', None),
            'agent_clipfracs': agent_info['update_info'].get('clipfracs', None),
