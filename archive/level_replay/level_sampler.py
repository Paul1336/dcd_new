...
class LevelSampler():
    ...
    def _update_with_rollouts(self, rollouts, score_function, external_scores=None):
        ...
        # masks's value is 1 if the step is not done, 0 if the step is done. Becase [[0.0] if done_ else [1.0] for done_ in done])
    ...
    def sample_replay_decision(self):
        if self.sample_full_distribution: 
            ...
            print('proportion_filled: ', proportion_filled)
            print('seed_buffer_size: ', self.seed_buffer_size)
            print('replay_schedule: ', self.replay_schedule)
            print('replay_prob: ', self.replay_prob)
            print('rho: ', self.rho)
            print('np.random.rand(): ', np.random.rand())
            print('min(proportion_filled, self.replay_prob): ', min(proportion_filled, self.replay_prob))
            print('self.replay_prob: ', self.replay_prob)
