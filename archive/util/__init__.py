...
from envs.iphyre.datapath import vlm_10k_task_dir_list, vlm_4k_task_dir_list 
from envs.iphyre.datapath import vlm_10k_claude_task_dir_list, vlm_10k_gemini_task_dir_list


def _make_env(args):
    env_kwargs = {'seed': args.seed, 'reset_random_after_episode': args.reset_random_after_episode}
    if args.env_name == 'Iphyre-AdversarialVLM4k-v0':
        env_kwargs.update({'env_names': args.current_env_names})
    elif args.env_name == 'Iphyre-AdversarialVLM10k-v0':
        env_kwargs.update({'env_names': args.current_env_names})
    elif args.env_name == 'Iphyre-AdversarialClaudeVLM10k-v0':
        env_kwargs.update({'env_names': args.current_env_names})
    elif args.env_name == 'Iphyre-AdversarialGeminiVLM10k-v0':
        env_kwargs.update({'env_names': args.current_env_names})
...
def create_parallel_env(args, adversary=True):
    ...
    is_iphyre = args.env_name.startswith('Iphyre')

    if args.env_name == 'Iphyre-AdversarialVLM4k-v0':
        task_dir_list = vlm_4k_task_dir_list

        env_names, _ = load_vlm_gen_tasks_solvable(
            task_dir_list, should_check_solvable=False
        )

        print(f"Total env ids: {len(env_names)} and we subsample to 4000")

        env_names = random.sample(env_names, 4000)
        args.current_env_names = env_names
    elif args.env_name == 'Iphyre-AdversarialVLM10k-v0':
        ...
        print(f"Total env ids: {len(env_names)} and we subsample to 10000")

        env_names = random.sample(env_names, 10000)
        args.current_env_names = env_names
    elif args.env_name == 'Iphyre-AdversarialClaudeVLM10k-v0':
        ...
        print(f"Total env ids: {len(env_names)} of Iphyre-AdversarialClaudeVLM10k-v0")
        args.current_env_names = env_names
    elif args.env_name == 'Iphyre-AdversarialGeminiVLM10k-v0':
        ...
        print(f"Total env ids: {len(env_names)} of Iphyre-AdversarialGeminiVLM10k-v0")
        args.current_env_names = env_names
    ...
    if is_multigrid or is_bipedalwalker or is_iphyre:
        ued_venv = venv
