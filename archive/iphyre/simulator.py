...
class IPHYRE:
    def __init__(self, game="support", fps=60, state_type="symbolic", frame_skip=1):
        # general information
        self.game = game
        ...
        self.set_game_configuration()

    def set_game_configuration(self):
    ...
    def add_static_line(self, l_pos, eli, friction, elasticity):
        ...
        static_shape = pymunk.Segment(
            static_body, (x1 - x, y1 - y), (x2 - x, y2 - y), 10
        )
    ...
    def add_dynamic_line(self, l_pos, friction, elasticity):
        ...
        x, y = (x1 + x2) / 2, (y1 + y2) / 2
        mass = 1.0
        moment = pymunk.moment_for_segment(mass, (0, 0), (0, 0), 10.0)
        body = pymunk.Body(mass, moment)
        body.position = x, y
        shape = pymunk.Segment(body, (x1 - x, y1 - y), (x2 - x, y2 - y), 10.0)
        shape.friction = friction
        shape.elasticity = elasticity
        self.space.add(body, shape)
        return body
    ...
    def add_joint(self):
        for b1, b2 in PARAS[self.game]["joint"]:
            c: pymunk.Constraint = pymunk.PinJoint(
                self.space.bodies[b1], self.space.bodies[b2]
            )
            self.space.add(c)
    ...
    def add_spring(self):
        for b1, b2 in PARAS[self.game]["spring"]:
            c = pymunk.DampedSpring(
                self.space.bodies[b1], self.space.bodies[b2], (0, 0), (0, 0), 20, 1, 0.3
            )
            self.space.add(c)

    def add_all(self):
        assert len(self.blocks) == len(PARAS[self.game]["eli"][: -self.num_ball])
        for l_para, eli, dynamics in zip(
            self.blocks,
            PARAS[self.game]["eli"][: -self.num_ball],
        ...
        for b_para in self.balls:
            self.add_ball(
                b_para[:2], b_para[2], self.b_mass, self.b_elasticity, self.b_friction
            )
        if self.joint:
            self.add_joint()
        if self.spring:
    ...
    def eliminate(self, p):
        for i, body in enumerate(self.space.bodies[: -self.num_ball]):
            ...
            if (
                self.eli[i] == 1
                and np.abs(np.dot(pv, lv)) < length / 2
                and np.abs(np.cross(pv, lv)) < 10
            ):
    ...
    def get_all_property(self):
        ...
        for i, body in enumerate(self.space.bodies):
            ...
            # eli_mask is the index of the body in the original list.
            # because the body may be eliminated, so we need to use eli_mask to get the index of the body in the original list.
    ...
    def step(self, pos):
        ...
        if self.state_type == "image":
            return self.get_image_state(), reward, done
        else:
    ...
    def simulate_vis(self, action=[]):
        ...
        while time_count < self.max_time:
            ...
            print(f"Step {step}: Time {time_count}. max_time {self.max_time}")
            for _ in range(self.frame_skip):
                self.space.step(self.timestep)
                time_count += self.timestep
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    if not finish_game and start:
                        self.eliminate(p)
            if start:
                # Please note that in play mode, we do not implement frame skip and action repeat.
                # Therefore, when playing by hand, the game will look more smooth.
                time_count += self.timestep
                ...
                self.add_text(text=f"Time: {time_count:.3f}", loc=(0, 0), color="black")

                pygame.display.flip()
                self.clock.tick(self.FPS)
            else:
                self.add_text(
                    text="Press s to start and r to reset.",
                    loc=(170, 0),
                    color="black",
                    font=30,
                )
                self.add_text(
                    text="Press n to move on and Quit to exit.",
                    loc=(125, 20),
                    color="black",
                    font=30,
                )
    ...
    def collect_seq_data(
        self, save_path="./offline_data/", act_lists=[], fps=10
    ):  # maximum fps=60
        """
        ...
        np.save(game_path + "raw.npy", dic)
        for i, act_list in enumerate(
            act_lists
        ):  # the step number of each action can be variant
            ...
            if not os.path.exists(data_path):
                ...
                np.save(
                    data_path + "actions.npy",
                    np.concatenate((act_pos, act_ts.reshape(-1, 1)), axis=-1),
                )
            else:
                continue  # already get the action_data stored
            step, time_count, save_count = 0, 0.0, 0
    ...
    def collect_while_play(
        self, player_name="", max_episode=5, save_path="./player_data.json"
    ):
        self.init_screen()
        self.reset()
        ...
        while time_count < self.max_time:
            ...
            if status == 0:
                self.add_text(
                    text=f"Press s to start!", loc=(230, 0), color="black", font=30
                )
                self.add_text(
                    text=f"Press e to end in advance!",
                    loc=(180, 20),
                    color="black",
                    font=30,
                )
                ...
                if reward not in [
                    self.step_reward,
                    self.success_reward + self.step_reward,
                ]:
                ...
                self.add_text(
                    text=f"Score in episode {episode} / {max_episode}: {round(total_reward, 2)}",
                    loc=(120, 0),
                    color="red",
                    font=40,
                )
                if episode < max_episode:
                    self.add_text(
                        text=f"Press r to reset!", loc=(230, 30), color="black", font=30
                    )
                else:
                    self.add_text(
                        text=f"Press n to move on to the next game.",
                        loc=(125, 30),
                        color="black",
                        font=30,
                    )
