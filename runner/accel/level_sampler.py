# Adapted from archive/level_replay/level_sampler.py
# Fixes applied for Python 3.8 / NumPy 1.19+:
#   - np.float  -> float
#   - np.bool   -> bool
#   - removed np.seterr(all='raise')
...
class LevelSampler():
    def __init__(
        ...
        if self.strategy.startswith('tscl'):
            ...
            self.unseen_seed_weights = np.zeros(N)
    ...
    def update_with_rollouts(self, rollouts, external_scores=None):
        ...
        elif self.strategy.startswith('tscl'):
    ...
    def _partial_update_seed_score(self, actor_index, seed, score, max_score, num_steps, done=False, running_mean=True):
        ...
        if done:
            self.partial_seed_scores[actor_index][seed_idx] = 0.
    ...
    def _tscl_window(self, **kwargs):
        ...
        x = list(self.tscl_episode_window[seed_idx])
        y = list(self.tscl_return_window[seed_idx])
